...
export default function Customers() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["fetch-all-customers", page, rowsPerPage, searchTerm],
    queryFn: ({ signal }) =>
      fetchCustomers({ signal, page, rowsPerPage, searchTerm }),
  ...
  const rows = data?.customers?.map((eachCustomer) =>
    createData(
      eachCustomer.id,
      eachCustomer.customerId,
      eachCustomer.name,
      eachCustomer.email,
      eachCustomer.address,
      eachCustomer.phoneNumber,
      eachCustomer.createdAt,
      eachCustomer.notes
    )
  );
