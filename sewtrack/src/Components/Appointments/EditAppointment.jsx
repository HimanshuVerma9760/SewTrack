...
export default function EditAppointment() {
  const id = useParams();
  const [name, setName] = useState(id.customerId);
  ...
  const {
    ...
    isLoading,
    isError: appointmentIdError,
    error: appointmentItemsError,
  } = useQuery({
    queryKey: ["fetch-all-appointmentItems", id.appointmentId],
    queryFn: ({ signal }) =>
      fetchAppointmentItems({ signal, appointmentId: id.appointmentId }),
    staleTime: 60 * 1000 * 60,
  });

  useEffect(() => {
    if (appointmentItemsData) {
      localStorage.setItem("appointmentId", id.appointmentId);
      setNoOfItemTypes(appointmentItemsData.length);
    }
  ...
  useEffect(() => {
    if (actionResponse && actionResponse.Success) {
      ...
      actionResponse &&
      !actionResponse.Success &&
      actionResponse.fieldError
    ) {
      toast.error("Kindly fill all fields properly");
  ...
  function setDynamicForm() {
    ...
    for (let i = 0; i < noOfItemTypes; i++) {
      ...
      setDynamicInput(itemInputs.current);
    }
  }
  ...
  } else if (appointmentIdError) {
    if (appointmentItemsError.info.status === 406) {
        ...
        <Form method="post">
          ...
          >
            ...
            {/* <TextField
              ...
              size="medium"
            /> */}
            {noOfItemTypes > 0 && dynamicInput.map((eachItem) => eachItem)}
            <TextField
              name="notes"
              id="notes"
              label="Notes"
